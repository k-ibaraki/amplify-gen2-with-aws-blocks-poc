...
- `AccountMenuBar` は **`user.username` を表示**する。Blocks 自前プールは `signInWith:'email'`＝**email-as-username** なので email 表示だが、**Amplify プールを `fromExisting` で借りると UUID username**（email は alias/属性）なので id 表示になり、mock（username=email）と食い違う。
- ＝これは公式 Blocks の標準挙動ではなく **hybrid（Amplify プール借用）特有の副作用**。
- 対処（UI 側）: `AccountMenuBar` を、`user.attributes.email` を表示する自前バーに差し替え。email は mock/Cognito 双方の user オブジェクトに存在するため、**プール再作成なしで両環境とも email 表示に統一**（mock・sandbox で確認済み）。
  - 別解（プール側）: Amplify プールを email-as-username 化（escape hatch ＋ プール再作成）。公式 Blocks と完全に揃うが email が不変識別子になる。今回は採らず。
