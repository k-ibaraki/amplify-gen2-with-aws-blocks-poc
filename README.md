...
> 📌 続編 **Phase 3**: この構成に **Amplify ネイティブの Auth（Cognito）を"足す" hybrid**（Blocks が `fromExisting` で
> 消費・API 認証必須化）を検証している。→ 後述「[認証](#認証amplify-ネイティブ-cognito-を-aws-blocks-が消費)」セクション。
...
- frontend は Blocks の `<Authenticator>`（モーダル）＋ 自前 `AccountBar`（email 表示）でログイン。**`aws-amplify` は使わない**（純 Blocks のまま）。
...
> 🔒 **本番の必須対応（CORS）**: 現在 backend は `CORS_ALLOWED_ORIGINS='.*'`（PoC）。認証 Cookie を使うため、
> ワイルドカードのままだと**任意サイトがユーザーの Cookie 付きで API を叩いて結果を読める**（CSRF/情報漏れ）。
> 本番では許可 origin を Amplify Hosting のドメインに必ず絞ること。
...
- **Phase 3（進行中）**: キレイに一本化した構成に **Amplify ネイティブの Auth（Cognito）を"足す"** hybrid。
  Blocks が `fromExisting` で消費し、API を認証必須化（循環参照なしを実機で確認済み）。**ローカル・Sandbox は検証済み**、本番のみ検証中。
...
| `src/App.tsx` | frontend（Blocks クライアント1本で Todo＋共有メモ／認証 UI＝自前 `AccountBar`＋`Authenticator` モーダル） |
